...
#plot showing home and away win ratio per team
ggplot(advdf3, aes(x = TEAM,)) + geom_point(aes(y = WIN_PCT_HOME, col = "wins HOME")) + geom_point(aes(y = WIN_PCT_AWAY, col = "wins  AWAY")) + ylab("Win ratio") + xlab("Team Names") 
```
...
The steps to calculate this measurement are as follows:  
    ...
    4. Perform a Chi-square test to examine whether there is a difference in the distribution for games won and games lost for the different restdays
    5. Calculating ratios for effects of rest days on win/loss form home/away
...
graph2 + labs(title = "Accuracy per second on shot clock left for Top Players vs Overall Average") + xlab("Seconds left on shot clock") + ylab("Accuracy")
...
####Correlation Test
  ...
  There is a medium to strong positive correlation between the defender distance and shooting accuracy. The Pearson’s correlation coefficient for the relationship between the variables "Defender Distance" and "Accuracy" was r(297) = .59, p < .01, with a level of significance of p = .05.  
...
simul_shooter <- sample(shot_results, size = sum(best_players_df[1,4], best_players_df[1,5], best_players_df[1,6]), replace = TRUE, prob = c(shooting_percentage[[1]], 1 - shooting_percentage[[1]]))
simul_shooter
...
set.seed(572)
shot_results <- c('made', 'missed')
simul_shooter <- sample(shot_results, size = sum(best_players_df[2,4], best_players_df[2,5], best_players_df[2,6]), replace = TRUE, 
...
set.seed(57)
shot_results <- c('made', 'missed')
simul_shooter <- sample(shot_results, size = sum(best_players_df[3,4], best_players_df[3,5], best_players_df[3,6]), replace = TRUE, 
...
###Discussion
  ...
  In the case of K. Thompson, his streak is seem as somewhat anomaly and further research is required to see if any similar results have been achieved by the player.
...
```{r, echo=FALSE}
graph01 <- ggplot(special2) + geom_boxplot(aes(x = player_name, y = FGA, colour = W)) + labs(title = "Differences in FGA for games lost/won for particular players") + xlab("Player name") + ylab("FGA")
graph02 <- ggplot(special2) + geom_boxplot(aes(x = player_name, y = FG.percentage, colour = W)) + labs(title = "Differences in FG% for games lost/won for particular players") + xlab("Player name") + ylab("FG%")
graph01
graph02
```
